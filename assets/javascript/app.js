$(document).ready(function() {
    ...
    $(".mCharacters").on("click", function() {
        console.log("Clicked");
        var display = $(this).attr("data-name");
        var queryURL = "http://api.giphy.com/v1/gifs/search?q=" + display + "rating=pg-13&limit=10&api_key=dc6zaTOxFJmzC";
        ...
        $.ajax({
            ...
            for (var i = 0; i < results.length; i++) {
                if (results[i].rating !== "r") {
                    ...
                    marvelDiv.append(pRate);
                    marvelDiv.append(marvelImage);

                    $(".characters").prepend(marvelDiv);
                }
            }
        });
    });
